package io.github.Project.game.systems.movementstrategy;
...
public class RocketMovementStrategy implements IMovementStrategy {
    ...
    /**
     * Sentinel ground entity used for collision callbacks.
     * Created lazily on first touchdown and reused — avoids allocating a new
     * Ground (and loading its texture) on every frame the rocket is on the ground.
     */
    ...
    /**
     * Clamps the rocket at ground level, fires the collision strategy once per
     * landing, applies friction, and resets the trigger when the rocket lifts off.
     */
    private void handleGroundCollision(Rocket rocket) {
        if (rocket.getPosY() <= 0) {
            if (!groundCollisionTriggered && rocket.getCollisionStrategy() != null) {
                groundCollisionTriggered = true;
                if (groundSentinel == null) groundSentinel = new Ground(0, -50, 400, 100);
                rocket.getCollisionStrategy().handleCollision(rocket, groundSentinel);
            }
            rocket.setPosY(0);
            if (rocket.getVy() < 0) rocket.setVy(0);
            rocket.setVx(rocket.getVx() * 0.85f);
        } else if (groundCollisionTriggered && rocket.getPosY() > 10f) {
